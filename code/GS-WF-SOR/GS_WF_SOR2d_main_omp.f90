...
! GS_WF_SOR2d_main_omp.f90

program GS_WF_SOR2d_main
    implicit none
    ...
    write (*,"('Solving Laplace equation on',i4,' by',i4,' grid by Gauss-Seidel-Wave-Front-SOR iteration')") n,n
    ! i4 - 4 characters wide
    ! print "('Solving Laplace equation on',i4,' by',i4,' grid by GS_WF_SOR iteration')", n,n
    ...
    ! nthreads = 1
    ! call GS_WF_SOR2d_sub(n, tol, maxiter, iter, dumax, nthreads)

    nthreads = 2
    call GS_WF_SOR2d_sub(n, tol, maxiter, iter, dumax, nthreads)


end program GS_WF_SOR2d_main
