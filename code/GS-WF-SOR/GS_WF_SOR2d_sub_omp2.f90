...
subroutine GS_WF_SOR2d_sub(n, tol, maxiter, iter, dumax, nthreads)
    ! Implements 2d GS_WF_SOR iteration on n by n grid
	...
	do i=1,n
	    ...
	    do k=1,i
		u(k,i-k+1) = w*0.25d0*(u(k-1,i-k+1) + uold(k+1,i-k+1) + &
                                 	u(k,i-k) + uold(k,i-k+2) + h**2*f(k,i-k+1)) + (1-w)*uold(k,i-k+1)
                dumax = max(dumax, abs(u(k,i-k+1)-uold(k,i-k+1)))
		enddo
	    enddo
	...
	do i=1,n-1
	    !$omp parallel do reduction(max : dumax)
	    do j=i,n-1
		u(j+1,n-j+i) = w*0.25d0*(u(j,n-j+i) + uold(j+2,n-j+i) + &
                                 	u(j+1,n-j+i-1) + uold(j+1,n-j+i+1) + h**2*f(j+1,n-j+i)) + (1-w)*uold(j+1,n-j+i)
                dumax = max(dumax, abs(u(j+1,n-j+i)-uold(j+1,n-j+i)))
		enddo
	    enddo
...
end subroutine GS_WF_SOR2d_sub
