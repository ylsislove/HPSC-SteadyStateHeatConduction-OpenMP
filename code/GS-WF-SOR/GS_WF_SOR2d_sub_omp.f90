...
subroutine GS_WF_SOR2d_sub(n, tol, maxiter, iter, dumax, nthreads)
    ...
    do iter=1,maxiter
        ...
        do j=1,2*n-1
	    !$omp parallel do private(k) reduction(max : dumax)
            do i=1,j
		k = j-i+1
		if ((k .le. n) .and. (k .ge. 0) .and. (i .le. n)) then
                	u(i,k) = w*0.25d0*(u(i-1,k) + uold(i+1,k) + &
                                 	u(i,k-1) + uold(i,k+1) + h**2*f(i,k)) + (1-w)*uold(i,k)
                	dumax = max(dumax, abs(u(i,k)-uold(i,k)))
			endif
            enddo
        enddo
