...
subroutine GS_RB_SOR2d_sub(n, tol, maxiter, iter, dumax, nthreads)
    ! Implements 2d GS_RB_SOR iteration on n by n grid
    ...
    ! GaussSeidel-RB-SOR iteratation:
    ...
    do iter=1,maxiter
        ...
        do j=1,n
           do i=mod(j,2)+1,n,2
               u(i,j) = w*0.25d0*(uold(i-1,j) + uold(i+1,j) + &
                                uold(i,j-1) + uold(i,j+1) + h**2*f(i,j)) + (1-w)*uold(i,j)
	...
	!$omp parallel do private(i) reduction(max : dumax)
        ! private - only be accessed by a single thread.
        do j=1,n
           do i=mod(j+1,2)+1,n,2
               u(i,j) = w*0.25d0*(u(i-1,j) + u(i+1,j) + &
                                 u(i,j-1) + u(i,j+1) + h**2*f(i,j)) + (1-w)*uold(i,j)
               dumax = max(dumax, abs(u(i,j)-uold(i,j)))
           enddo
        enddo
...
end subroutine GS_RB_SOR2d_sub
