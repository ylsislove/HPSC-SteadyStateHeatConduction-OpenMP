...
subroutine GS_WF2d_sub(n, tol, maxiter, iter, dumax, nthreads)
    ! Implements 2d GS_WF iteration on n by n grid
    ...
    do iter=1,maxiter
		...
		if ((k .le. n) .and. (k .ge. 0) .and. (i .le. n)) then
                	u(i,k) = 0.25d0*(u(i-1,k) + uold(i+1,k) + &
                                 	u(i,k-1) + uold(i,k+1) + h**2*f(i,k))
